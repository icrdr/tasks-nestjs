...
export class EditorBinding {
  ...
  constructor(editor: EditorJS, yArray: Y.Array<any>, debug: boolean = false) {
    this.editor = editor;
    this.yArray = yArray;
    this.debug = debug;
  ...
  private async initialize() {
    ...
    if (this.debug) console.log(`${this.yArray.length} yBlocks ready to init`);
  ...
  yInsertDeleteHandler(event: Y.YEvent) {
    ...
    // if (this.debug) console.log(delta);
    let startIndex = 0;
      ...
      switch (key) {
        ...
        case "insert":
          ...
          for (const yBlock of yBlocks) {
            ...
            if (this.debug) console.log(`yEvent[insert]: ${uuid}`);
            this.eApi.insert(
              yBlock.get("type"),
              yBlock.get("data"),
              null,
        ...
        case "delete":
          ...
          this.eEventLocks.set(uuid, true);
          if (this.debug) console.log(`yEvent[delete]: ${uuid}`);
      ...
      if (key === "data") {
        switch (change.action) {
          case "update":
            const uuid = yBlock.get("uuid");
            this.eEventLocks.set(uuid, true);
            if (this.debug) console.log(`yEvent[update]: ${uuid}`);
  ...
  private async eEventCallback(events: Set<eEvent>) {
    for await (const { uuid, type } of events) {
      ...
      // TODO: when switching blocks after update editor compounent. the editor became empty.
      ...
      this.mux(() => {
        if (this.debug) console.log(`eEvent[${type}]: ${uuid}`);
        const yBlock =
