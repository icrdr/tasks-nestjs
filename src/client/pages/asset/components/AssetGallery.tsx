...
import { AssetListRes, AssetRes, ChangeAssetDTO, GetAssetsDTO } from '@dtos/asset.dto';
...
const AssetGallery: React.FC<{
  ...
  const menu = (asset: AssetRes) => {
    return (
      <Menu>
        ...
        {isEdit && [
          <Menu.Item key="2" onClick={() => setEditingAssetId(asset.id)}>
            重命名
          </Menu.Item>,
          <Menu.Item key="3">
            <Popconfirm
  ...
  const changeAsset = (id: number, body: ChangeAssetDTO) => {
  ...
  const beforeItemRender = async (item) => {
    ...
    if (item.preview) {
      ...
      item['_source'] =
        _preview[0] === 'oss' ? oss.signatureUrl(_preview[1], { expires: 3600 }) : _preview[1];
    ...
    <Dropdown overlay={() => menu(item)} trigger={['contextMenu']}>
      <div>
        <FileCard
          ...
          onPressEnter={(v) => changeAssetReq.run(item.id, { name: v })}
          onTapPreview={() => {
            const i = vGalleryRef.current?.items.map((asset) => asset.id).indexOf(item.id);
            setLightBoxSlide(i);
  ...
  return (
    <>
      ...
      <FsLightbox
        ...
        key={lightBoxUpdate}
        // disableThumbs={true}
        sources={vGalleryRef.current?.items.filter((a) => !!a).map((asset) => asset['_source'])}
        types={vGalleryRef.current?.items.filter((a) => !!a).map((asset) => asset['_type'])}
      />
    </>
  );
};
