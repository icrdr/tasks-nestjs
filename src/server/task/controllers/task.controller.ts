...
export class TaskController {
  ...
  async changeTask(
    ...
    return new TaskMoreDetailRes(await this.taskService.changeTask(task, user, body), user);
  }
  ...
  async saveTaskContent(@CurrentUser() user: User, @TargetTask() task: Task) {
    return new TaskMoreDetailRes(await this.taskService.saveTaskContent(task, user), user);
  }
  ...
  async commitTask(@CurrentUser() user: User, @TargetTask() task: Task) {
    return new TaskMoreDetailRes(await this.taskService.commitOnTask(task, user), user);
  }
  ...
  async acceptCommit(@CurrentUser() user: User, @TargetTask() task: Task) {
    return new TaskMoreDetailRes(await this.taskService.refuseToCommit(task, user),user);
  }
}
